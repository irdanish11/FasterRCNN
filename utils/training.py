...
from network.anchors import AnchorsGenerator
from network.frcnn import FasterRCNN, FastRCNNPredictor
...
def create_model(num_classes):
    ...
    if backbone_network == 'mobilenet':
        backbone = MobileNetV2(
            weights_path=cfg.backbone_pretrained_weights).features
        backbone.out_channels = 1280

        roi_pooler = ops.MultiScaleRoIAlign(featmap_names=['0'],
                                            # roi pooling in which resolution feature
                                            output_size=cfg.roi_out_size,
                                            # roi_pooling output feature size
...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        print('{} Total time: {} ({:.0f} s / it)'.format(header,
                                                         total_time_str,
